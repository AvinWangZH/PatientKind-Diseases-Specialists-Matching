...
for row_num in positive_val_row_num:
    if not np.any(np.isnan(positive_set[row_num, :])):
        X_positive_val.append(list(np.concatenate((positive_set[row_num, 0:1], positive_set[row_num, 3:]), axis=0)))
        X_positive_val_names.append(positive_set_names_temp[row_num])
    if count == 1080:
        break
    count += 1
X_positive_1 = np.array(X_positive_val)
...
for row_num in negative_val_row_num:
    if not np.any(np.isnan(negative_set[row_num, :])):
        X_negative_val.append(list(np.concatenate((negative_set[row_num, 0:1], negative_set[row_num, 3:]), axis=0)))
        X_negative_val_names.append(negative_set_names_temp[row_num])
    if count == 1080:
        break
    count += 1
X_negative_1 = np.array(X_negative_val)
...
for row_num in positive_test_row_num:
    if not np.any(np.isnan(positive_set[row_num, :])):
        X_positive_test.append(list(np.concatenate((positive_set[row_num, 0:1], positive_set[row_num, 3:]), axis=0)))
        X_positive_test_names.append(positive_set_names_temp[row_num])
    if count == 1080:
        break  
    count += 1
X_positive_2 = np.array(X_positive_test)
logging.info('X_positive_test has been finished')

count = 1  
for row_num in negative_test_row_num:
    if not np.any(np.isnan(negative_set[row_num, :])):
        X_negative_test.append(list(np.concatenate((negative_set[row_num, 0:1], negative_set[row_num, 3:]), axis=0)))
        X_negative_test_names.append(negative_set_names_temp[row_num])
    if count == 1080:
        break 
    count += 1
X_negative_2 = np.array(X_negative_test)
logging.info('X_negative_test has been finished')

for row_num in negative_training_row_num:
    if not np.any(np.isnan(negative_set[row_num, :])):
        X_train.append(list(np.concatenate((negative_set[row_num, 0:1], negative_set[row_num, 1:]), axis=0)))
...
n_samples = X_positive_1.shape[0] + X_negative_1.shape[0] + X_positive_2.shape[0] + X_negative_2.shape[0]
X = np.vstack((X_positive_1, X_negative_1, X_positive_2, X_negative_2))

# generate labels
a = list(np.ones(X_positive_1.shape[0]))
b = list(np.zeros(X_negative_1.shape[0]))
c = list(np.ones(X_positive_2.shape[0]))
d = list(np.zeros(X_negative_2.shape[0]))
a.extend(b)
a.extend(c)
a.extend(d)
labels = np.array(a)
...
#C_values = [0.01, 0.03, 0.1, 0.3, 1, 3, 10, 30, 100, 300, 1000, 3000]
...
for train_index, test_index in kf:
    ...
    logging.info('Random Num: {}'.format(1))
